...
public:
	int bulbSwitch(int n) {
		...
		那么下一步就是求i(i≤n)的约数个数，想到这里，要发现一点，约数是成对存在的，即2是8的约数，那么 8÷2=4也是8的约数，其中有一种特殊情况，就是i为完全平方数，比如9跟它的约数3,因此，
		如果i是完全平方数，那么i的约数个数肯定是奇数，如果i不是完全平方数，由于约数成对出现，所以约数个数肯定是偶数。
		想出了这一点，这道题就变成了更简单的题目：计算1-n中完全平方数的数目

		为什么对于n以内的数只含有sqrt(n)个完全平方数?
